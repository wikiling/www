...
module Compiler.Inference where
...
newtype TypeEnv = TypeEnv (Map.Map Syn.SVar Scheme)
  deriving Monoid

data Unique = Unique { count :: Int }

type Infer = ExceptT TypeError (State Unique)
type Subst = Map.Map Syn.TVar Type

data TypeError
  = UnificationFail Syn.Type Syn.Type
  | InfiniteType Syn.TVar Syn.Type
  | UnboundVariable String

runInfer :: Infer (Subst, Syn.Type) -> Either Ty.TypeError Scheme
runInfer m = case evalState (runExceptT m) initUnique of
  Left err  -> Left err
  Right res -> Right $ closeOver res

closeOver :: (Map.Map Syn.TVar Syn.Type, Syn.Type) -> Scheme
...
extend :: TypeEnv -> (Syn.SVar, Scheme) -> TypeEnv
extend (TypeEnv env) (x, s) = TypeEnv $ Map.insert x s env

emptyTyenv :: TypeEnv
emptyTyenv = TypeEnv Map.empty

typeof :: TypeEnv -> Syn.SVar -> Maybe Type.Scheme
typeof (TypeEnv env) name = Map.lookup name env

class Substitutable a where
  apply :: Subst -> a -> a
  ftv   :: a -> Set.Set Syn.TVar
